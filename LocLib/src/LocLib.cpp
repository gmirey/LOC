...
local_func_inl void acquire_task_manager_lock(WholeProgramCompilationState* pCompState, WorkerDesc* pWorkerDesc)
{
    // TODO !!
}

local_func_inl void release_task_manager_lock(WholeProgramCompilationState* pCompState, WorkerDesc* pWorkerDesc)
{
    // TODO !!
}

local_func void while_filewise_task_locked_awake_tasks_waiting_for(TCWaitingReason waitingReason,
    SourceFileDescAndState* pSourceFile, WholeProgramCompilationState* pCompState, WorkerDesc* pWorker,
    TmpArray<int>* outVecSourceFilesWithNoMoreWaitingTasks, TmpArray<int>* outVecSourceFilesWithPossiblyNewTasksToLaunch)
{
    auto itWaitingForThisReason = pSourceFile->mapWaitingTasksByReason.find(waitingReason);
    if (itWaitingForThisReason != pSourceFile->mapWaitingTasksByReason.end()) {
        u32 uCount = itWaitingForThisReason.value().size();
        Assert_(uCount);
        BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED("Awaking %u task(s) in file '%s'",
            u64(uCount),
            reinterpret_cast<u64>(pSourceFile->sourceFileName.c_str())), pWorker);
        for (u32 uIndex = 0; uIndex < uCount; uIndex++) {
            TCContext* pTCContextThere = itWaitingForThisReason.value()[uIndex];
            Assert_(pTCContextThere->pIsolatedSourceFile == pSourceFile);
            pSourceFile->vecTCTasksToLaunch.append(pTCContextThere);
            Assert_(pTCContextThere->pNamespace);
            Assert_(pTCContextThere->pNamespace->pOriginalSourceFile == pSourceFile);
            if (pTCContextThere->eKind <= ETypecheckContextKind::ECTXKIND_GLOBAL_PRIVATE) {
                TCNamespace* pNamespace = pSourceFile->vecNamespaces[pTCContextThere->pNamespace->uRegistrationIndex];
                Assert_(pNamespace->uCountGlobalTasksInWaitingTasks);
                pNamespace->uCountGlobalTasksInWaitingTasks--;
                pNamespace->uCountGlobalTasksInTasksToLaunch++;
            }
        }
        pSourceFile->mapWaitingTasksByReason.remove_at_iter(itWaitingForThisReason);
        if (pSourceFile->mapWaitingTasksByReason.size() == 0u)
            outVecSourceFilesWithNoMoreWaitingTasks->append(pSourceFile->iRegistrationIndex);
        outVecSourceFilesWithPossiblyNewTasksToLaunch->append(pSourceFile->iRegistrationIndex);
    }
}

local_func void signal_update_namespace_using_while_locked(TCNamespace* pNamespace,
    WholeProgramCompilationState* pCompState, WorkerDesc* pWorker)
{
    ArenaRefPoint refBefore = get_arena_ref_point(pWorker->tmpArena);
    TmpStackOptiArray<SourceFileDescAndState*, 64u> vecSourceFilesWithWaitingTasks(pWorker->tmpArena);
    u32 uCountFiles = pCompState->setOfFilesWithTCWaitingTasks.size();
    vecSourceFilesWithWaitingTasks.reserve(uCountFiles);
    for (auto it = pCompState->setOfFilesWithTCWaitingTasks.begin(), itEnd = pCompState->setOfFilesWithTCWaitingTasks.end();
         it != itEnd; it++) {
        vecSourceFilesWithWaitingTasks.append(pCompState->vecSourceFiles[*it]);
    }
    // TODO: is this unlock really okay here ? it would be if we ensure we only process once such *signal* at a time,
    // ie. something able to mess with *Waiting tasks*. But we currently don't. TODO
    release_task_manager_lock(pCompState, pWorker);

    //TmpStackOptiArray<int, 64u> vecSourceFilesWithNoMoreWaitingTasks(pWorker->tmpArena);
    TmpStackOptiArray<int, 64u> vecSourceFilesWithPossiblyNewTasksToLaunch(pWorker->tmpArena);

    // TODO: CLEANUP: Currently, in case *any* task reports a new 'using' discovered on *any* namespace, we throw our hands
    //   in the air and wake-up *all waiting tasks throughout the program*. TODO: improve our ability to cross-ref only those
    //   tasks concerned.
    BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED("Notifying discovery of new 'using' on a namespace. Warning: Currently Waking ALL. TODO:CLEANUP"), pWorker);

    Assert_(uCountFiles == vecSourceFilesWithWaitingTasks.size());
    for (u32 uIndex = 0u; uIndex < uCountFiles; uIndex++) {
        SourceFileDescAndState* pSourceFile = vecSourceFilesWithWaitingTasks[uIndex];
        acquire_source_file_specific_task_lock(pSourceFile, pWorker);
        for (auto it = pSourceFile->mapWaitingTasksByReason.begin(), itEnd = pSourceFile->mapWaitingTasksByReason.end();
                  it != itEnd; it++) {
            u32 uCount = it.value().size();
            Assert_(uCount);
            BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED("Awaking %u task(s) in file '%s'",
                u64(uCount),
                reinterpret_cast<u64>(pSourceFile->sourceFileName.c_str())), pWorker);
            for (u32 uIndex = 0; uIndex < uCount; uIndex++) {
                TCContext* pTCContextThere = it.value()[uIndex];
                Assert_(pTCContextThere->pIsolatedSourceFile == pSourceFile);
                pSourceFile->vecTCTasksToLaunch.append(pTCContextThere);
                Assert_(pTCContextThere->pNamespace);
                Assert_(pTCContextThere->pNamespace->pOriginalSourceFile == pSourceFile);
            ...
            pSourceFile->mapWaitingTasksByReason.clear();
            Assert_(pSourceFile->mapWaitingTasksByReason.size() == 0u);
            vecSourceFilesWithPossiblyNewTasksToLaunch.append(pSourceFile->iRegistrationIndex);
        }
        release_source_file_specific_task_lock(pSourceFile, pWorker);
    }

    if (vecSourceFilesWithPossiblyNewTasksToLaunch.size() == 0) {
        BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED("No TC tasks awoken for this event"), pWorker);
    }

    acquire_task_manager_lock(pCompState, pWorker);
    pCompState->setOfFilesWithTCWaitingTasks.clear(); // !!
    u32 uCountToPossiblyAdd = vecSourceFilesWithPossiblyNewTasksToLaunch.size();
    for (u32 uToAdd = 0u; uToAdd < uCountToPossiblyAdd; uToAdd++) {
        int iToPossiblyAdd = vecSourceFilesWithPossiblyNewTasksToLaunch[uToAdd];
        if (pCompState->setOfFilesWithActiveTasksBeingRun.find(iToPossiblyAdd) == pCompState->setOfFilesWithActiveTasksBeingRun.end()) {
            pCompState->setOfFilesWithTCTasksToLaunch.insert(vecSourceFilesWithPossiblyNewTasksToLaunch[uToAdd]);
        }
    }
    reset_arena_no_release_to(refBefore, pWorker->tmpArena);
}

local_func void signal_global_ids_available_while_locked(TCNamespace* pNamespace, SetView<int> setIds,
    WholeProgramCompilationState* pCompState, WorkerDesc* pWorker)
{
    ArenaRefPoint refBefore = get_arena_ref_point(pWorker->tmpArena);
    TmpStackOptiArray<SourceFileDescAndState*, 64u> vecSourceFilesWithWaitingTasks(pWorker->tmpArena);
    ...
    Assert_(setIds.size() > 0u);
    BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED("Notifying discovery of bindings for %u distinct identifiers",
        u64(setIds.size())), pWorker);

    // CLEANUP: We currently wake *all* tasks waiting for this particular identifier, even if not waiting for it within that particular
    //   namespace. So we could improve things a bit here, even if not as urgent as improving thigs for the 'using' case, above.
    Assert_(uCountFiles == vecSourceFilesWithWaitingTasks.size());
    for (u32 uIndex = 0u; uIndex < uCountFiles; uIndex++) {
        SourceFileDescAndState* pSourceFile = vecSourceFilesWithWaitingTasks[uIndex];
        acquire_source_file_specific_task_lock(pSourceFile, pWorker);
        for (auto it = setIds.begin(), itEnd = setIds.end();
              it != itEnd; it++) {
            int iIdentifier = *it;
            TCWaitingReason becauseWaitingForThisId = make_waiting_reason(
                ETaskWaitingReason::EWR_GLOBAL_IDENTIFIER_RESOLUTION, WAITING_REASON_NO_SPECIFIC_INDEX, u32(iIdentifier));
            while_filewise_task_locked_awake_tasks_waiting_for(becauseWaitingForThisId, pSourceFile, pCompState, pWorker,
                &vecSourceFilesWithNoMoreWaitingTasks, &vecSourceFilesWithPossiblyNewTasksToLaunch);
        }
        release_source_file_specific_task_lock(pSourceFile, pWorker);
    }
    ...
    acquire_task_manager_lock(pCompState, pWorker);
    u32 uCountToRemove = vecSourceFilesWithNoMoreWaitingTasks.size();
    for (u32 uToRemove = 0u; uToRemove < uCountToRemove; uToRemove++) {
        pCompState->setOfFilesWithTCWaitingTasks.remove_at_key(vecSourceFilesWithNoMoreWaitingTasks[uToRemove]);
    }
    u32 uCountToPossiblyAdd = vecSourceFilesWithPossiblyNewTasksToLaunch.size();
    for (u32 uToAdd = 0u; uToAdd < uCountToPossiblyAdd; uToAdd++) {
        int iToPossiblyAdd = vecSourceFilesWithPossiblyNewTasksToLaunch[uToAdd];
...
local_func void signal_proc_body_state_change_while_locked(SourceFileDescAndState* pDeclSourceFile, TCProcBodyResult* pProcBody,
    u8 uProcBodyStatus, WholeProgramCompilationState* pCompState, WorkerDesc* pWorker)
{
    WRITE_FENCE();
    InterlockedExch32(&(pProcBody->uProcBodyTypechekingStatus), uProcBodyStatus);
    ...
    int iIdentifier = pProcBody->iPrimaryIdentifier;
    u32 uRegistrationIndex = pProcBody->uRegistrationIndex;
    int iRegisteredInFile = pProcBody->iSourceFileIndex;
    int iSourceFileId = pDeclSourceFile->iRegistrationIndex;
    Assert_(iRegisteredInFile == iSourceFileId);
    BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED("Changing Proc-Body typeckecking status to %u for proc %u in file '%s'",
        u64(uProcBodyStatus), u64(pProcBody->uRegistrationIndex),
        reinterpret_cast<u64>(pDeclSourceFile->sourceFileName.c_str())), pWorker);

    TCWaitingReason becauseWaitingForThisProc = make_waiting_reason(
            ETaskWaitingReason::EWR_PROC_BODY_TYPECHECK_SUCCESS, iRegisteredInFile, uRegistrationIndex);

    Assert_(uCountFiles == vecSourceFilesWithWaitingTasks.size());
    for (u32 uIndex = 0u; uIndex < uCountFiles; uIndex++) {
        SourceFileDescAndState* pSourceFile = vecSourceFilesWithWaitingTasks[uIndex];
        acquire_source_file_specific_task_lock(pSourceFile, pWorker);
        while_filewise_task_locked_awake_tasks_waiting_for(becauseWaitingForThisProc, pSourceFile, pCompState, pWorker,
            &vecSourceFilesWithNoMoreWaitingTasks, &vecSourceFilesWithPossiblyNewTasksToLaunch);
        release_source_file_specific_task_lock(pSourceFile, pWorker);
    }
...
local_func void signal_compound_state_change_while_locked(SourceFileDescAndState* pDeclSourceFile, TypeInfo_CompoundBase* pCompoundType,
    u32 uCompoundTCStatus, WholeProgramCompilationState* pCompState, WorkerDesc* pWorker)
{
    WRITE_FENCE();
    InterlockedExch32(&(pCompoundType->pRegistration->uTCProgress), uCompoundTCStatus);
    ...
    TCCompoundRegistration* pCompoundRegistration = pCompoundType->pRegistration;
    u32 uRegistrationIndex = pCompoundType->uRegistrationIndex;
    int iRegisteredInFile = pCompoundRegistration->pStatementWithSignature->iSourceFileIndex;
    int iIdentifier = pCompoundRegistration->iPrimaryIdentifier;
    Assert_(pDeclSourceFile->vecAllCompoundDef[uRegistrationIndex] == pCompoundRegistration);
    int iSourceFileId = pDeclSourceFile->iRegistrationIndex;
    BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED("Changing compound typeckecking status to %u for compound %u in file '%s'",
        u64(uCompoundTCStatus), u64(uRegistrationIndex),
        reinterpret_cast<u64>(pDeclSourceFile->sourceFileName.c_str())), pWorker);

    TCWaitingReason becauseWaitingForThisCompound = make_waiting_reason(
        ETaskWaitingReason::EWR_COMPOUND_BODY_TYPECHECK_SUCCESS, iRegisteredInFile, uRegistrationIndex);

    Assert_(uCountFiles == vecSourceFilesWithWaitingTasks.size());
    for (u32 uIndex = 0u; uIndex < uCountFiles; uIndex++) {
        SourceFileDescAndState* pSourceFile = vecSourceFilesWithWaitingTasks[uIndex];
        acquire_source_file_specific_task_lock(pSourceFile, pWorker);
        while_filewise_task_locked_awake_tasks_waiting_for(becauseWaitingForThisCompound, pSourceFile, pCompState, pWorker,
            &vecSourceFilesWithNoMoreWaitingTasks, &vecSourceFilesWithPossiblyNewTasksToLaunch);
        release_source_file_specific_task_lock(pSourceFile, pWorker);
    }
    ...
    WholeProgramCompilationState* pCompState, LocLib_CompilationResults* oCompilationResults)
{
    ...
    if (bOk) {
        ...
        pSourceFile->vecTCTasksToLaunch.append(pTCContextToPush);
        ...
        pSourceFile->pRootNamespace->uCountGlobalTasksInTasksToLaunch = 1u;
        release_source_file_specific_task_lock(pSourceFile, pWorkerDesc);
    }

    acquire_task_manager_lock(pCompState, pWorkerDesc);
    if (bOk) {
        pCompState->setOfFilesWithTCTasksToLaunch.insert(iSourceFileIndexToParse);
        pCompState->setOfFilesWithActiveTasksBeingRun.remove_at_key(iSourceFileIndexToParse);
        pCompState->uCountOfActivelyWorking--; // TODO: atomic
    } else {
        pCompState->setOfFilesWithActiveTasksBeingRun.remove_at_key(iSourceFileIndexToParse);
        pCompState->uCountOfActivelyWorking--; // TODO: atomic
    }
    release_task_manager_lock(pCompState, pWorkerDesc);
}

local_func void on_procbody_tc_task_succeeded_return_once_locked_task_manager(TCContext* pTCContext,
    WholeProgramCompilationState* pCompState, LocLib_CompilationResults* oCompilationResults)
{
    ...
    acquire_task_manager_lock(pCompState, pTCContext->pWorker);
    signal_proc_body_state_change_while_locked(pTCContext->pIsolatedSourceFile, pTCContext->pProcResult, EPBTC_DONE, pCompState, pTCContext->pWorker);
}

local_func void on_procbody_tc_task_on_hold_return_once_locked_task_manager(TCContext* pTCContext,
    WholeProgramCompilationState* pCompState, LocLib_CompilationResults* oCompilationResults)
{
    // NOOP
    acquire_task_manager_lock(pCompState, pTCContext->pWorker);
...
local_func void on_procbody_tc_task_on_error_return_once_locked_task_manager(TCContext* pTCContext,
    WholeProgramCompilationState* pCompState, LocLib_CompilationResults* oCompilationResults)
{
    Assert_(pTCContext->pProcResult);

    acquire_task_manager_lock(pCompState, pTCContext->pWorker);
    signal_proc_body_state_change_while_locked(pTCContext->pIsolatedSourceFile, pTCContext->pProcResult, EPBTC_ON_ERROR, pCompState, pTCContext->pWorker);
}

local_func void on_compound_tc_task_possibly_succeeded_return_once_locked_task_manager(TCContext* pTCContext,
    WholeProgramCompilationState* pCompState, LocLib_CompilationResults* oCompilationResults)
{
    // ended a Typecheck compound-type-body kind...
    Assert_(pTCContext->pCompoundToTC);
    TypeInfo_CompoundBase* pCompoundType = pTCContext->pCompoundToTC->pCompoundType;
    ETypeKind eTypeKind = get_type_kind(pCompoundType);

    ECompoundTCProgress progressToSignal = ECOMPOUND_NOT_YET_STARTED; // by default, this is our marker for "no signalling", in this func

    if (eTypeKind == ETYPEKIND_STRUCTLIKE) { // structs and unions req. runtime sizing
        if (pCompoundType->_coreType & COMPOUNDTYPE_IS_COMPTIME_ONLY) {
            // TODO: CLEANUP signalling and progress in the presence of error cases
            Assert(false, "signalling TC progress of comptime-only structs not yet implemented");
        } else {
            if (0 == (pCompoundType->_coreFlags & COMPOUNDFLAG_BODY_IN_ERROR_RUNTIME)) {
                Assert_(0 == (pCompoundType->_coreFlags & COMPOUNDFLAG_BODY_IN_ERROR)); // ???? TMP assert to try and see what's happening here
                if (pTCContext->pCompoundToTC->uTCProgress < ECOMPOUND_DONE_RUNTIME) {
                    if (pTCContext->pCompoundToTC->setWaitingPossiblyRuntime.size() == 0) {
                        if (try_finalize_structlike_runtime_tc((TypeInfo_StructLike*)pCompoundType, pTCContext)) {
                            if (pTCContext->pCompoundToTC->setWaitingConstOnly.size() == 0)
                                progressToSignal = ECOMPOUND_DONE_ALL;
                            else
                                progressToSignal = ECOMPOUND_DONE_RUNTIME;
                        } else {
                            BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED(
                                "*** Trying to finalize compound returned false!!"), pTCContext->pWorker);
                            if (pCompoundType->_coreFlags & COMPOUNDFLAG_BODY_IN_ERROR_RUNTIME)
                                progressToSignal = ECOMPOUND_DONE_ALL;
                            else {
                                BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED(
                                    "*** Warning : Trying to finalize compound returned false, without the compound being in error. This case is not yet properly handled"),
                                    pTCContext->pWorker);
                            }
                        }
                    } else {
                        BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED(
                            "Still %u possibly-runtime waiting task(s) on compound '%s'",
                            u64(pTCContext->pCompoundToTC->setWaitingPossiblyRuntime.size()),
                            reinterpret_cast<u64>(get_identifier_string(pCompState, pTCContext->pCompoundToTC->iPrimaryIdentifier).c_str())), pTCContext->pWorker);
                    }
                } else if (pTCContext->pCompoundToTC->uTCProgress == ECOMPOUND_DONE_RUNTIME) {
                    if (pTCContext->pCompoundToTC->setWaitingConstOnly.size() == 0) {
                        BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED(
                            "*** Warning : Done with a compound, but we're not the ones having set up its state to done ?"), pTCContext->pWorker);
                        progressToSignal = ECOMPOUND_DONE_ALL;
                    } else {
                        BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED(
                            "Still %u const waiting task(s) on compound '%s'",
                            u64(pTCContext->pCompoundToTC->setWaitingConstOnly.size()),
                            reinterpret_cast<u64>(get_identifier_string(pCompState, pTCContext->pCompoundToTC->iPrimaryIdentifier).c_str())), pTCContext->pWorker);
                    }
                }

            } else {
                if (pTCContext->pCompoundToTC->uTCProgress < ECOMPOUND_DONE_ALL) {
                    BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED(
                        "*** Finalized a compound, in an error state..."), pTCContext->pWorker);
                    progressToSignal = ECOMPOUND_DONE_ALL;
                }
            }
        }

    } else { // Finalizing enum types: much simpler, as they are sequential...
    ...
    acquire_task_manager_lock(pCompState, pTCContext->pWorker);
    if (progressToSignal != ECOMPOUND_NOT_YET_STARTED)
        signal_compound_state_change_while_locked(pTCContext->pIsolatedSourceFile, pCompoundType, progressToSignal, pCompState, pTCContext->pWorker);
}

local_func void on_compound_tc_task_on_hold_return_once_locked_task_manager(TCContext* pTCContext,
    WholeProgramCompilationState* pCompState, LocLib_CompilationResults* oCompilationResults)
{
    // NOOP
    acquire_task_manager_lock(pCompState, pTCContext->pWorker);
...
local_func void on_compound_tc_task_on_error_return_once_locked_task_manager(TCContext* pTCContext,
    WholeProgramCompilationState* pCompState, LocLib_CompilationResults* oCompilationResults)
{
    Assert_(pTCContext->pCompoundToTC);
    TypeInfo_CompoundBase* pCompoundType = pTCContext->pCompoundToTC->pCompoundType;
    pCompoundType->_coreFlags |= COMPOUNDFLAG_BODY_IN_ERROR|COMPOUNDFLAG_BODY_IN_ERROR_RUNTIME; // TODO: differentiate runtime errors!

    acquire_task_manager_lock(pCompState, pTCContext->pWorker);
    signal_compound_state_change_while_locked(pTCContext->pIsolatedSourceFile, pCompoundType, ECOMPOUND_DONE_ALL, pCompState, pTCContext->pWorker);
}

local_func void on_global_tc_task_ended_return_once_locked_task_manager(TCContext* pTCContext,
    WholeProgramCompilationState* pCompState, LocLib_CompilationResults* oCompilationResults)
{
    pTCContext->pNamespace->uCountGlobalTasksInTasksToLaunch -= 1u;
    if (pTCContext->pNamespace->uCountGlobalTasksInTasksToLaunch == 0u && pTCContext->pNamespace->uCountGlobalTasksInWaitingTasks == 0u) {
        if (pTCContext->pNamespace->eCompState != ESourceCompState::ESOURCE_COMP_STATE_DONE_LOCAL_DISCOVERY) {
            pTCContext->pNamespace->eCompState = ESourceCompState::ESOURCE_COMP_STATE_DONE_LOCAL_DISCOVERY;
            BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED(
                "Could notify end of discovery for all globals (barring using) in namespace %u of file '%s'",
            ...
            // TODO: check 'using' others. If for all, also done using discovery => ESOURCE_COMP_STATE_DONE_USING_DISCOVERY
            // TODO: check 'used by' others. Tell them that they may update to ESOURCE_COMP_STATE_DONE_USING_DISCOVERY if we were last waited.
        }
    }
    ...
    if (pTCContext->pNamespace->mapOthersUsingThis.size() != pTCContext->pNamespace->asRef.laggedState.mapKnownOthersUsingThis.size()) {
        Assert_(pTCContext->pNamespace->mapOthersUsingThis.size() > pTCContext->pNamespace->asRef.laggedState.mapKnownOthersUsingThis.size());
        bUpdateUsedBy = true;
    }
    bool bHasNewlyDeclaredIds = pTCContext->pNamespace->setOfNewlyDeclaredGlobalIdentifiers.size() > 0;
    if (bHasNewlyDeclaredIds || bUpdateUsing) {
        BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED(
            "Updating state of namespace %u of file '%s'",
            u64(pTCContext->pNamespace->uRegistrationIndex),
            reinterpret_cast<u64>(pTCContext->pIsolatedSourceFile->sourceFileName.c_str())), pTCContext->pWorker);
        acquire_global_using_graph_lock(pTCContext->pProgCompilationState, pTCContext->pWorker);
        acquire_write_laggued_state_lock(pTCContext->pIsolatedSourceFile, pTCContext);
        write_namespace_to_self_lagged_state_while_under_lock(pTCContext->pNamespace, bHasNewlyDeclaredIds, bUpdateUsing||bUpdateUsedBy,
            pTCContext->pIsolatedSourceFile->localArena); 
        release_write_laggued_state_lock(pTCContext->pIsolatedSourceFile, pTCContext);
        release_global_using_graph_lock(pTCContext->pProgCompilationState, pTCContext->pWorker);
    }

    acquire_task_manager_lock(pCompState, pTCContext->pWorker);
    if (bUpdateUsing) {
        signal_update_namespace_using_while_locked(pTCContext->pNamespace, pCompState, pTCContext->pWorker);
        pTCContext->pNamespace->setOfNewlyDeclaredGlobalIdentifiers.clear();
    } else if (bHasNewlyDeclaredIds) {
        signal_global_ids_available_while_locked(pTCContext->pNamespace, pTCContext->pNamespace->setOfNewlyDeclaredGlobalIdentifiers,
            pCompState, pTCContext->pWorker);
        pTCContext->pNamespace->setOfNewlyDeclaredGlobalIdentifiers.clear();
    }
}
    ...
    WholeProgramCompilationState* pCompState, LocLib_CompilationResults* oCompilationResults)
{
    ...
    Assert_(pSourceFile->vecTCTasksToLaunch.size()); // No need to sync since only one at a time ever working on this file
    TCContext* pTCContext = pSourceFile->vecTCTasksToLaunch.last();
    pSourceFile->vecTCTasksToLaunch.remove_last();
    ...
    for (auto it = pSourceFile->mapReferencedOtherSources.begin(),
        ...
        ReferencedOtherSource* pOther = it.value();
        // TODO: lock read lagged state on pOther->pSourceFile
        if (pOther->laggedState.uVersion < pOther->pSourceFile->laggedState.uVersion) {
            pOther->laggedState = pOther->pSourceFile->laggedState;
        }
        // TODO: unlock read lagged state on pOther->pSourceFile
    }
    #endif

    ETCResult result = start_or_resume_typecheck_task(pTCContext);

    if (pTCContext->eKind == ECTXKIND_PROCBODY) { Assert_(pTCContext->pProcResult);
        if (result == ETCResult::ETCR_SUCCESS)
            on_procbody_tc_task_succeeded_return_once_locked_task_manager(pTCContext, pCompState, oCompilationResults);
        else if (result == ETCResult::ETCR_WAITING)
            on_procbody_tc_task_on_hold_return_once_locked_task_manager(pTCContext, pCompState, oCompilationResults);
        else { Assert_(result == ETCResult::ETCR_ERROR);
        ...
        if (get_type_kind(pTCContext->pCompoundToTC->pCompoundType) == ETypeKind::ETYPEKIND_STRUCTLIKE) {
            ...
            if (itFindAsWaitingPossiblyRuntime != pTCContext->pCompoundToTC->setWaitingPossiblyRuntime.end()) {
                pTCContext->pCompoundToTC->setWaitingPossiblyRuntime.remove_at_iter(itFindAsWaitingPossiblyRuntime);
            } else {
                auto itFindAsWaitingConst = pTCContext->pCompoundToTC->setWaitingConstOnly.find(pTCContext);
                if (itFindAsWaitingConst != pTCContext->pCompoundToTC->setWaitingConstOnly.end()) {
                    pTCContext->pCompoundToTC->setWaitingConstOnly.remove_at_iter(itFindAsWaitingConst);
                }
            }
        }

        if (result == ETCResult::ETCR_SUCCESS)
            on_compound_tc_task_possibly_succeeded_return_once_locked_task_manager(pTCContext, pCompState, oCompilationResults);
        else if (result == ETCResult::ETCR_WAITING)
            on_compound_tc_task_on_hold_return_once_locked_task_manager(pTCContext, pCompState, oCompilationResults);
        else { Assert_(result == ETCResult::ETCR_ERROR);
            on_compound_tc_task_on_error_return_once_locked_task_manager(pTCContext, pCompState, oCompilationResults);
        }
    } else { Assert_(pTCContext->eKind <= ETypecheckContextKind::ECTXKIND_GLOBAL_PRIVATE);
        on_global_tc_task_ended_return_once_locked_task_manager(pTCContext, pCompState, oCompilationResults);
    }

    // At this point, we should be under lock...

    if (pTCContext->pIsolatedSourceFile->vecTCTasksToLaunch.size() == 0 && pTCContext->pIsolatedSourceFile->mapWaitingTasksByReason.size() == 0) {
        if (pTCContext->pIsolatedSourceFile->pRootNamespace->eCompState != ESourceCompState::ESOURCE_COMP_STATE_DONE_TYPECHECKED) {
            BLOCK_TRACE(ELOCPHASE_REPORT, _LLVL6_SIGNIFICANT_INFO, EventREPT_CUSTOM_HARDCODED(
                "Could notify end of all TC tasks for file '%s'",
                reinterpret_cast<u64>(pSourceFile->sourceFileName.c_str())), pTCContext->pWorker);
        }
    }

    if (pSourceFile->mapWaitingTasksByReason.size()) {
        pCompState->setOfFilesWithTCWaitingTasks.insert(iSourceFileIndexToTC);
    } else {
        pCompState->setOfFilesWithTCWaitingTasks.remove_at_key(iSourceFileIndexToTC);
    }

    pCompState->setOfFilesWithActiveTasksBeingRun.remove_at_key(iSourceFileIndexToTC);
    if (pSourceFile->vecTCTasksToLaunch.size()) {
        pCompState->setOfFilesWithTCTasksToLaunch.insert(iSourceFileIndexToTC);
    }
    pCompState->uCountOfActivelyWorking--; // TODO: atomic ?
    release_task_manager_lock(pCompState, pWorkerDesc);
}
    ...
    while (true) {
        ...
        if (pWorkerDesc->bAllowTC) { // a priori all of them... TODO: maybe have a priority system instead ?
            if (pCompState->setOfFilesWithTCTasksToLaunch.size()) {
                ...
                pCompState->setOfFilesWithTCTasksToLaunch.remove_at_iter(it);
                release_task_manager_lock(pCompState, pWorkerDesc);
        ...
        if (pCompState->uCountOfActivelyWorking > 0) { // Should work without sync if atomic
            release_task_manager_lock(pCompState, pWorkerDesc);

            handle_put_worker_to_sleep(pWorkerDesc, pCompState);
            continue;
...
local_func void emit_waiting_task_errors(ArrayView<TCContext*> vecWaiting, u16 uErrCode, const char* formatMsg)
{
    u32 uCount = vecWaiting.size();
    Assert_(uCount);
    for (u32 uIndex = 0; uIndex < uCount; uIndex++) {
        TCContext* pSleepingContext = vecWaiting[uIndex];
        u32 uCurrentStatement = should_tc_ctx_halt_on_non_success(pSleepingContext) ?
            pSleepingContext->pCurrentBlock->uStatementBeingTypechecked : pSleepingContext->uGlobalStatementOnHold;
        TCStatement* pTCStatement = pSleepingContext->pCurrentBlock->vecStatements[uCurrentStatement];
        TmpTCNode waitingNode = init_tmp_tc_node(pSleepingContext->uNodeIndexWithWait, pTCStatement, pSleepingContext);
        emit_error(&waitingNode, pTCStatement, pSleepingContext, uErrCode, formatMsg);
    }
}

local_func void emit_unsolved_identifier_errors(ArrayView<TCContext*> vecWaiting)
{
    emit_waiting_task_errors(vecWaiting, RERR_UNRESOLVED_IDENTIFIER, "unresolved identifier");
}

local_func void emit_unfinished_tc_procbody_errors(ArrayView<TCContext*> vecWaiting)
{
    emit_waiting_task_errors(vecWaiting, RERR_STILL_SLEEPING_WAITPROCBODY, "continuation waiting for a still unsolved procbody typecheck");
}

local_func void emit_unfinished_tc_compound_errors(ArrayView<TCContext*> vecWaiting)
{
    emit_waiting_task_errors(vecWaiting, RERR_STILL_SLEEPING_WAITCOMPOUND, "continuation waiting for a still unsolved compound-type");
}

local_func void emit_unfinished_gathermore_errors(ArrayView<TCContext*> vecWaiting)
{
