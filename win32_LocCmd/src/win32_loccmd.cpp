...
int wmain( int argc, wchar_t *argv[ ], wchar_t *envp[ ] )
{
	...
	RunLocCompilerFromFirstFile_Sign pRunLocCompilerFn = 0;
    ReleaseCompilationResults_Sign pReleaseCompResultsFn = 0;
    InitReportLanguage_Sign pInitReportLanguageFn = 0;
    ReleaseReportLanguage_Sign pReleaseReportLanguageFn = 0;
    SetPlatformConfsFn* pSetPlatformConfsFn;

	HMODULE hLocDLL = LoadLibraryA("..\\build\\loclib.dll");
	if (hLocDLL) {
		pRunLocCompilerFn = (RunLocCompilerFromFirstFile_Sign)GetProcAddress(hLocDLL, "run_loc_compiler_from_first_file");
        pReleaseCompResultsFn = (ReleaseCompilationResults_Sign)GetProcAddress(hLocDLL, "release_compilation_results");
        pInitReportLanguageFn = (InitReportLanguage_Sign)GetProcAddress(hLocDLL, "init_report_language");
        pReleaseReportLanguageFn = (ReleaseReportLanguage_Sign)GetProcAddress(hLocDLL, "release_report_language");
        pSetPlatformConfsFn = (SetPlatformConfsFn*)GetProcAddress(hLocDLL, "set_platform_confs");
		...
		if (!pRunLocCompilerFn || !pReleaseCompResultsFn || !pInitReportLanguageFn || !pReleaseReportLanguageFn || !pSetPlatformConfsFn ||
			...
			log_info("Invalid LOC library (loclib.dll) in 'build' directory: Missing expected functions");
			return -4;
		}
	} else {
    ...
    int iUtf8FilePathBytes = convert_path_to_utf8(tFilePathUTF8Buffer, 32767, argv[1], UNKNOWN_SIZE_BEFORE_UTF_CONVERSION);
    if (iUtf8FilePathBytes <= 0) {
		log_info("Could not convert source file path to UTF8");
		return -5;
    }

    std::string sFirstFilePathUtf8 = std::string(tFilePathUTF8Buffer);
    u16 uError = 0;
    ...
    g_mapIndexRegisteredByPathFromRoot[sFirstFilePathUtf8] = 0;
    ...
    theAllocConf.page_alloc = page_alloc;
    theAllocConf.page_free = page_free;
    PlatformLoggingConf theLoggingConf;
    theLoggingConf.on_report_error_to_user_should_break = on_report_error_to_user_should_break;
    ...
    theIOConf.load_file_entirely = read_file_entirely_utf8;
    theIOConf.release_whole_file_buffer = free_file_memory;
    theIOConf.exit_process = exit_process;
    theIOConf.open_file_for_writing = open_file_for_writing;
    theIOConf.write_to_file = write_to_file;
    theIOConf.close_file = close_file;

    pSetPlatformConfsFn(theAllocConf, theLoggingConf, theIOConf);
    ...
    if (pRunLocCompilerFn(0, &g_osWrapperFunctions, &compilationParams, &compilationResults)) {
	    if (compilationResults.uErrorCount) {
            log_info("compilation carried out with errors:");
            u32 uFileCount = compilationResults.uFileCount;
            if (uFileCount) {
                int iFirstFileWithError = compilationResults.iFirstFileWithError;
                bool bNeedsToDetailFirstError = true;
                if (iFirstFileWithError >= 0) {
                    SourceFileDescAndState* pFileDesc = compilationResults.tFiles[iFirstFileWithError];
                    report_errors(iFirstFileWithError, pFileDesc, true, &compilationResults, &g_osWrapperFunctions);
                    bNeedsToDetailFirstError = false;
                }
                for (u32 uFile = 0; uFile < uFileCount; uFile++) {
                    if (int(uFile) != iFirstFileWithError) {
                        SourceFileDescAndState* pFileDesc = compilationResults.tFiles[uFile];
                        u64 uErrorCount; g_pGetErrorsOnSourceFileFn(pFileDesc, &uErrorCount);
                        if (uErrorCount) {
                            report_errors(int(uFile), pFileDesc, bNeedsToDetailFirstError, &compilationResults, &g_osWrapperFunctions);
                            bNeedsToDetailFirstError = false;
                        }
                    }
                }
            } else {
                log_info("***fatal error : no file found for error report");
            }
        } else if (compilationResults.uWarningCount) {
            log_info("compilation carried out with warnings:");
            // TODO: log warnings
        } else 
            log_info("compilation carried out successfully");
        // TODO
        // ... or maybe not TODO: we could release comp results, but closing the program would do as well...
        //pReleaseCompResultsFn(&compilationResults, &osWrapperFunctions);
    } else {
	    log_info("***fatal error : invalid arguments to run_loc_compiler_from_first_file");
        return -1;
    }
	
    // TMP TMP TMP TMP
    /*
    const char* sHelloThere = "Hello, There\n";
    const char* sLaBaguette = "Là baguét (no spec)\n";
    const wchar_t* sLaBaguetteU16 = L"Là baguét (u16)\n";
    const char* sLaBaguetteU8 = u8"Là baguét (u8)\n";

    HANDLE stdOut = GetStdHandle(STD_OUTPUT_HANDLE);
    WriteConsoleA(stdOut, sHelloThere, strlen(sHelloThere), 0, 0);
    WriteConsoleA(stdOut, sLaBaguette, strlen(sLaBaguette), 0, 0);
    WriteConsoleW(stdOut, sLaBaguetteU16, wcslen(sLaBaguetteU16), 0, 0);
    WriteConsoleA(stdOut, sLaBaguetteU8, strlen(sLaBaguetteU8), 0, 0);
    const char* sNowSwitchingCP = "Now switching to CP 65001:\n";
    WriteConsoleA(stdOut, sNowSwitchingCP, strlen(sNowSwitchingCP), 0, 0);
    UINT uPrevCP = GetConsoleOutputCP();
    ...
    const char* sNowSwitchingBack = "Now switching back to previous CP.\n";
    WriteConsoleA(stdOut, sNowSwitchingBack, strlen(sNowSwitchingBack), 0, 0);
    SetConsoleOutputCP(uPrevCP);
    */
    // TMP TMP TMP TMP
